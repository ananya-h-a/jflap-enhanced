...
public class Automaton implements Serializable, Cloneable {
	public boolean fromShowAnswer;
	public boolean fromOriginalAnswer;
	public boolean fromSaveProblem;
	public boolean showOnTop;
	public boolean fromSolve;
	...
	public ConvertFSAToREActionNew cfrReference;
...
//				dest.addTransition(ts[i].copy(from, to));
	...
	public void replaceTransition(Transition oldTrans, Transition newTrans) {
		...
		/* Set change to true to enable detecting change of a transition 
		 * And set add to false*/
	...
	public State createState(String anotherId) {
		int i = 0;
		while (getStateWithID(i) != null)
			i++;
		State state = new State(i, anotherId,this);
		addState(state);
		return state;
	}
...
//		}
	...
	public String toString() {
		...
		{
			buffer.append("<State><Id>"+states[s].id+"</Id>");
			if (initialState == states[s])
				buffer.append("<Attribute>Initial State</Attribute>");
			//buffer.append(states[s]);
			if (isFinalState(states[s]))
				buffer.append("<Attribute>Final State</Attribute>");
			buffer.append("</State>\n");
		...
		{
			buffer.append("<Transition><From>"+t.getFromState().getID()+"</From>");
			buffer.append("<Label>"+ ((FSATransition)t).getLabel()+ "</Label>");
			buffer.append("<To>"+ t.getToState().getID()+"</To></Transition>\n");
