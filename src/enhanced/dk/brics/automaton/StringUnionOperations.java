package enhanced.dk.brics.automaton;
...
final public class StringUnionOperations {
	 ...
	 */
	private static enhanced.dk.brics.automaton.State convert(State s, 
			IdentityHashMap<State, enhanced.dk.brics.automaton.State> visited) {
		enhanced.dk.brics.automaton.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new enhanced.dk.brics.automaton.State();
	 ...
	 */
	public static enhanced.dk.brics.automaton.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, enhanced.dk.brics.automaton.State>());
	}
